...
La caracterización léxica confirma además que el shiwilu presenta una densidad morfológica mayor que el castellano dentro del corpus, reflejada en una proporción más alta de tipos únicos y hapax. Esta observación justifica dos decisiones metodológicas: evitar una normalización destructiva que borre información morfológica, y preparar un modelo de \emph{embeddings} capaz de operar como recurso semántico auxiliar. En línea con la literatura de lenguas de bajo recurso, el valor principal del recurso no se limita al volumen absoluto, sino a su control, trazabilidad y posibilidad de comparación experimental.
...
NLLB-200 no incluye Shiwilu en su inventario de lenguas. La extensión del \emph{tokenizer} registra \texttt{shw\_Latn} como un \emph{special token} adicional, actualiza los mapas \texttt{lang\_code\_to\_id} e \texttt{id\_to\_lang\_code}, y, por compatibilidad con tareas multitarea, registra también las etiquetas estilo T5 \texttt{<2shw>} y \texttt{<2spa>}. La fila de \emph{embeddings} correspondiente al nuevo identificador se inicializa como la media (en \texttt{float32}) de las filas asociadas a tres lenguas indígenas sudamericanas que NLLB sí soporta: \texttt{quy\_Latn} (Quechua de Ayacucho), \texttt{ayr\_Latn} (Aymara) y \texttt{grn\_Latn} (Guarani). La elección responde a contigüidad geográfica, propiedades fonológicas y prosódicas compatibles con el shiwilu de la familia cahuapana, y disponibilidad efectiva de cobertura de subpalabras en NLLB.
