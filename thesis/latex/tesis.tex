...
Como resultado de este capítulo, la tesis dispone de un corpus paralelo consolidado de 4814 pares útiles provenientes de ocho fuentes bilingües complementarias, particiones reproducibles para experimentación y un modelo de \emph{embeddings} bilingüe evaluado en ambas direcciones. Estos elementos constituyen la base empírica sobre la cual se apoyan las siguientes etapas de modelado y validación.
...
\section{Arquitectura general}\label{nmt-arquitectura}
...
\section{Datos canónicos para NMT}\label{nmt-datos}
...
\section{Filtrado semántico de calidad}\label{nmt-filtrado}
...
\section{Entrenamiento del modelo \texorpdfstring{\(v_0\)}{v0}}\label{nmt-entrenamiento}
...
\section{Augmentación de datos para \texorpdfstring{\(v_{1bt}\)}{v1bt}}\label{nmt-aumentacion}
...
\section{Resultados experimentales}\label{nmt-resultados}
...
El protocolo, implementado por \filepath{scripts/nmt/71\_human\_eval\_template.py}, se ejecuta con los parámetros: \texttt{--per-direction 100}, \texttt{--seed 2026}, comparando los cuatro sistemas (\(v_0\) y \(v_{1bt}\), cada uno en variantes \emph{baseline} y \emph{reranked}) sobre el conjunto de \emph{test}. El muestreo es estratificado por origen (\emph{flashcards} vs \emph{pdf\_textos} vs otras fuentes) y por \emph{bucket} de longitud (corto \(\leq 5\), medio \(\leq 12\), largo \(>12\) palabras).

Las hipótesis se anonimizan con un mapeo aleatorio letra \(\to\) sistema (A, B, C, D), persistido en \filepath{reports/05\_nmt/evaluation\_xl/human\_eval\_anon\_key.json}. La planilla resultante \filepath{human\_eval\_template.csv} contiene, por fila, la fuente, la referencia, las cuatro hipótesis anonimizadas y tres celdas vacías para que el revisor consigne puntajes 1--5 en las dimensiones de adecuación, fluidez y pertinencia cultural.
