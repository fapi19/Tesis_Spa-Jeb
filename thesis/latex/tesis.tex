...
La Tabla~\ref{tab:materiales-bilingues} resume la composición inicial validada después de los procesos de filtrado y extracción. Esta tabla reemplaza los conteos preliminares usados durante etapas anteriores por los valores consolidados del pipeline actual, que ahora cubre las ocho fuentes habilitadas en \texttt{config/sources.json}.
...
La auditoría permitió describir el corpus tanto en términos cuantitativos como en términos de calidad. El corpus unificado contiene 4894 registros provenientes de las ocho fuentes habilitadas; de ellos, 80 fueron excluidos por duplicación exacta antes de generar las particiones canónicas. Por tanto, el corpus final utilizado para entrenamiento y evaluación de recursos intermedios quedó compuesto por 4814 pares paralelos.
...
\subsection{R2: modelo de embeddings preparado y evaluado}\label{preparacion-del-modelo-de-embeddings}
...
El modelo de \emph{embeddings} v3 ofrece una señal robusta para recuperación bilingüe en ambas direcciones, con R@1 cercano a ocho de cada diez consultas sobre oraciones completas. Este resultado cumple el IOV asociado a R2 porque el recurso no solo queda entrenado, sino medido con particiones fijas y reportes reproducibles. Sin embargo, su desempeño no debe interpretarse como validación semántica perfecta: los ítems de vocabulario atómico y las equivalencias culturalmente marcadas siguen siendo más difíciles que las oraciones completas, por lo que la verificación humana continúa siendo necesaria para usos finales.

En conjunto, el capítulo cumple el OE1 mediante dos resultados verificables: R1, un corpus paralelo consolidado, depurado y particionado; y R2, un modelo de \emph{embeddings} bilingüe preparado y evaluado. El \hyperref[anexo-documento-reproducible]{Anexo B} contiene los medios de verificación que permiten reproducir el pipeline, revisar criterios de inclusión y normalización, consultar manifiestos de partición y auditar las métricas del modelo de representación.

\chapter{Sistema de traducción neuronal SA-BiNLLB}\label{nmt-sa-binllb}

\section{Introducción}\label{nmt-introduccion}

A partir del corpus paralelo shiwilu--castellano consolidado en el Capítulo~\ref{recopilaciuxf3n-y-preparaciuxf3n-inicial-del-corpus-bilinguxfce} y del modelo de \emph{embeddings} bilingüe congelado en ese mismo capítulo, este capítulo describe el sistema de traducción automática neuronal implementado en este trabajo: \textbf{SA-BiNLLB} (\emph{Semantic-Aware Bidirectional NLLB}). El nombre resume las tres decisiones de diseño que estructuran al sistema: (i) un \emph{backbone} multilingüe pre-entrenado (NLLB-200) adaptado al par shiwilu--castellano mediante una capa LoRA bidireccional liviana; (ii) la reutilización del modelo de \emph{embeddings} bilingüe en tres puntos del pipeline (filtrado de datos, reranking de hipótesis y filtrado de pares sintéticos por consistencia), lo cual da el carácter \emph{semantic-aware} al sistema; y (iii) una capa de aumentación de datos cuya admisión se decide a través del mismo evaluador semántico, evitando así la circularidad de que el modelo NMT valide sus propios datos sintéticos.
...
Este capítulo reporta los resultados asociados al segundo objetivo específico: implementar y evaluar un sistema de traducción automática neuronal bidireccional para el par shiwilu--castellano. Los entregables centrales son R3, el sistema shiwilu\(\rightarrow\)castellano, y R4, el sistema castellano\(\rightarrow\)shiwilu, ambos implementados dentro de una arquitectura única SA-BiNLLB y medidos con el mismo protocolo automático sobre particiones fijas. El \hyperref[anexo-nmt-reproducible]{Anexo~C} funciona como medio de verificación reproducible de hiperparámetros, scripts, métricas, ablaciones, intervalos de confianza, análisis de rare-token y plantilla de evaluación humana preparada.
...
El tercer bloque es el reutilizador semántico. El modelo de \emph{embeddings} bilingüe v3 congelado en el Capítulo~\ref{recopilaciuxf3n-y-preparaciuxf3n-inicial-del-corpus-bilinguxfce} se incorpora al sistema NMT en tres puntos: como filtro de calidad sobre el conjunto de entrenamiento (Sección~\ref{nmt-filtrado}), como filtro de consistencia sobre los pares sintéticos generados por backtranslation (Sección~\ref{nmt-aumentacion}) y como reranker de las hipótesis devueltas por la búsqueda en haz (Sección~\ref{nmt-resultados}). Es esta triple reutilización lo que justifica el nombre \emph{semantic-aware}: el sistema NMT no opera en aislamiento sino acoplado a un evaluador semántico independiente y reutilizable. Los hiperparámetros exactos de cada bloque (rangos, tasas, fórmulas, umbrales) se desarrollan en el \hyperref[anexo-nmt-reproducible]{Anexo~C}, secciones~\ref{anexo-nmt-hp} y~\ref{anexo-nmt-tokenizer}.
...
\begin{table}[h]
\centering
\caption{\emph{Leaderboard} cerrado del ciclo de ablaciones sobre la variante \texttt{xl} (446 pares por dirección, 892 filas direccionales). chrF++ es la métrica primaria; BLEU y COMET se reportan como métricas complementarias.}
...
Tres lecturas adicionales emergen del Cuadro~\ref{tab:nmt-leaderboard} y del Cuadro~\ref{tab:nmt-bootstrap-ci}. Primero, LoRA+ aislado (\(v_{2.1b}\)) es el ganador del ciclo con avg chrF++ reranked \(=44.99\) (IC 95\,\% \([43.17, 46.96]\)), BLEU promedio \(=18.46\) y COMET promedio \(=0.752\); el salto de \(\approx 12{,}8\) puntos chrF++ sobre \(v_{1bt}\) es estadísticamente claro porque los intervalos de confianza no se traslapan. Segundo, DoRA aislado (\(v_{2.0}\)) no se distingue estadísticamente de \(v_{1bt}\) (\(32{,}58\) vs \(32{,}14\), IC 95\,\% traslapados), y la combinación DoRA+LoRA+ (\(v_{2.1}\), \(44{,}30\)) cae dentro del IC del propio LoRA+ aislado, de modo que DoRA no paga su costo computacional adicional con este corpus. Tercero, \(v_{2.2}\) regresa el modelo a \(\approx 32\) chrF++ y también baja en BLEU/COMET respecto de \(v_{2.1b}\): la \emph{backtranslation} desde Wikipedia (registro enciclopédico, vocabulario externo al corpus de \emph{flashcards} y narrativas comunitarias) introduce pares sintéticos cuya distribución de dominio inunda la señal limpia de LoRA+, confirmando empíricamente que en \emph{low-resource} la coincidencia de dominio prima sobre el volumen del pool monolingüe.

Por estas tres lecturas el ciclo de tesis cierra con \(v_{2.1b}\) LoRA+ como sistema enviado. Los detalles operativos de DoRA, LoRA+ y la generación del pool monolingüe iterativo se documentan en el Anexo~C, sección~\ref{anexo-nmt-ablaciones-peft}.
...
Este capítulo desarrolla el objetivo específico 3: definir un protocolo de validación que combine evaluación automática y validación participativa con hablantes para medir adecuación lingüística y pertinencia cultural en el sistema shiwilu$\leftrightarrow$castellano. A diferencia del capítulo anterior, centrado en la implementación y evaluación automática del modelo NMT bidireccional, aquí el resultado principal es documental y operativo: dejar preparado un procedimiento reproducible para que las métricas automáticas se interpreten junto con juicios humanos estructurados.

El alcance corresponde al resultado R5 y a sus medios de verificación MV5/IOV5. Por tanto, el capítulo no reporta una aplicación ya ejecutada con hablantes, ni presenta promedios humanos, actas de expertos o acuerdos interevaluador. Lo que se documenta es el protocolo, la muestra, la rúbrica, los artefactos generados y las condiciones necesarias para aplicar la validación participativa en una etapa posterior.

\section{Resultados alcanzados}\label{nmt-eval-humana}

El protocolo integra dos componentes. El primero es la evaluación automática ya ejecutada sobre el conjunto de prueba, con BLEU, chrF++ y COMET por dirección y por variante del sistema. La tabla de liderazgo del Cuadro~\ref{tab:nmt-leaderboard} y los intervalos bootstrap del Cuadro~\ref{tab:nmt-bootstrap-ci} funcionan como base cuantitativa: permiten identificar el sistema ganador, comparar el efecto del reranking y estimar la incertidumbre de chrF++ sobre 446 pares por dirección. Los reportes reproducibles asociados son \filepath{reports/05\_nmt/evaluation\_xl/leaderboard.md} y \filepath{reports/05\_nmt/evaluation\_xl/bootstrap\_ci\_summary.md}.

El segundo componente es la plantilla de validación participativa preparada para revisión con hablantes. La muestra contiene 100 ítems por dirección, 200 en total, extraídos del conjunto de prueba mediante muestreo estratificado por fuente de origen y por longitud de la oración fuente. Esta decisión evita que la revisión se concentre únicamente en frases cortas o en una sola fuente del corpus, y mantiene trazabilidad con la distribución real de los datos evaluados automáticamente.

La plantilla compara cuatro salidas del sistema: \(v_0\), \(v_0\) con reranking, \(v_{1bt}\) y \(v_{1bt}\) con reranking. Para reducir sesgo de evaluación, las hipótesis se presentan anonimizadas como A/B/C/D, mientras que la clave letra\(\rightarrow\)sistema se guarda por separado en \filepath{reports/05\_nmt/evaluation\_xl/human\_eval\_anon\_key.json}. La hoja de revisión queda disponible en \filepath{reports/05\_nmt/evaluation\_xl/human\_eval\_template.csv}; cada fila incluye la fuente, la referencia y las hipótesis anonimizadas. Además, el resumen operativo del protocolo queda exportado en \filepath{reports/05\_nmt/evaluation\_xl/human\_eval\_protocol.md} y el directorio cuenta con un \filepath{README.md} que ordena los artefactos automáticos y participativos.

La rúbrica propuesta usa escala 1--5 en tres dimensiones:

\begin{description}[leftmargin=*]
  \item[Adecuación.] Evalúa si la hipótesis conserva el sentido de la referencia y evita omisiones, añadidos o cambios semánticos relevantes.
  \item[Fluidez.] Evalúa gramaticalidad, naturalidad y legibilidad de la salida en la lengua destino.
  \item[Pertinencia cultural.] Evalúa si la traducción conserva registro, usos idiomáticos y formulaciones culturalmente apropiadas para el shiwilu y el castellano.
\end{description}

El artefacto se reproduce con el siguiente comando desde la raíz del proyecto:

\begin{quote}
\texttt{.venv-nmt/Scripts/python -m scripts.nmt.71\_human\_eval\_template --variant xl --per-direction 100 --seed 2026 --split test}
\end{quote}

Con ello, R5 queda documentado como protocolo preparado: existen métricas automáticas reportadas, muestra participativa estratificada, rúbrica de evaluación, plantilla de captura, clave anónima separada y resumen Markdown reproducible. La ejecución con hablantes, el cálculo de promedios por sistema, el análisis cualitativo de comentarios y la medición de acuerdo entre evaluadores no forman parte de los resultados ejecutados de esta etapa.

\section{Discusión}\label{protocolo-discusion}

Contar con este protocolo resuelve una brecha metodológica importante para lenguas de muy bajo recurso: evita que la calidad del traductor se interprete únicamente desde indicadores automáticos y deja preparada una ruta para incorporar juicio lingüístico y cultural. BLEU permite mantener comparabilidad con literatura previa, chrF++ ofrece una señal más robusta ante variación morfológica y COMET aporta una estimación semántica complementaria; sin embargo, ninguna de estas métricas sustituye la revisión de hablantes. BLEU es sensible a coincidencias superficiales, chrF++ sigue dependiendo de cercanía de caracteres respecto de una referencia, y COMET no fue entrenado específicamente para shiwilu.

La validación participativa propuesta responde a esa limitación porque separa tres criterios que las métricas tienden a mezclar: preservación del sentido, naturalidad de la salida y pertinencia cultural. Esta separación es especialmente relevante en traducción de lenguas originarias, donde una hipótesis puede aproximarse a la referencia en superficie pero fallar en registro, cortesía, uso idiomático o elección léxica culturalmente marcada. La anonimización A/B/C/D también fortalece el diseño, porque reduce la posibilidad de que el evaluador favorezca una variante por conocer su configuración técnica.

El principal límite es que el protocolo aún no ha sido aplicado con hablantes. Por ello, el capítulo no permite afirmar que el sistema ya fue validado participativamente ni que existe acuerdo entre evaluadores. Su aporte es dejar el instrumento preparado y reproducible para que la aplicación posterior pueda realizarse sin modificar la muestra, la rúbrica ni la clave de anonimización. En términos de trazabilidad, el OE3 queda cumplido en su componente protocolar y documental, mientras que la ejecución de sesiones, consolidación de puntajes humanos y análisis de discrepancias se mantienen como trabajo futuro.
...
Respecto del OE2, se implementó y evaluó un sistema NMT bidireccional basado en NLLB-200 distilled, adaptación LoRA, aumentación controlada, reranking semántico y ablaciones de optimización. El mejor resultado del ciclo fue \(v_{2.1b}\) con LoRA+ y reranking, que alcanzó avg chrF++ \(=44{,}99\) con IC 95\,\% \([43{,}17, 46{,}96]\). Las pruebas muestran que la mejora depende tanto del corpus \texttt{xl} como de la calidad de los datos sintéticos y del optimizador; también muestran que la backtranslation fuera de dominio puede degradar el modelo aun cuando aumente el volumen de entrenamiento.

Respecto del OE3, se definió un protocolo que combina evaluación automática y validación participativa. El componente automático quedó respaldado por \emph{leaderboard}, métricas por dirección e intervalos bootstrap; el componente participativo quedó preparado mediante una muestra estratificada de 100 ítems por dirección, una rúbrica 1--5 de adecuación, fluidez y pertinencia cultural, una plantilla CSV y una clave anónima separada. Este resultado corresponde a R5 y debe interpretarse como protocolo documentado/preparado, no como validación con hablantes ya ejecutada.

La investigación confirma que, para una lengua originaria de muy bajo recurso como el shiwilu, la contribución técnica más defendible es un pipeline reproducible y auditable antes que una afirmación de cobertura general. El sistema obtenido constituye una línea base fuerte para continuar el trabajo, pero sus resultados deben interpretarse dentro de sus límites: corpus pequeño, dominio concentrado en flashcards y narrativas, baja recuperación de tokens OOV, dependencia de métricas automáticas y validación participativa todavía pendiente.

Por esa razón, la validación participativa completa y el prototipo funcional no se declaran como resultados ejecutados. En esta etapa queda preparada la plantilla de evaluación humana y documentado el protocolo, pero no se reportan juicios de hablantes, promedios humanos, acuerdos interevaluador ni actas de expertos. Del mismo modo, no se redacta un prototipo como resultado alcanzado, porque su implementación queda pendiente para la siguiente fase.
...
\begin{description}[leftmargin=*]
  ...
  \item[\emph{Stochastic Weight Averaging} sobre el ganador.] Promediar los pesos de los últimos 3 a 5 \emph{checkpoints} del modelo ganador \parencite{izmailov2018swa} suele aportar entre \(+0{,}3\) y \(+0{,}8\) chrF++ adicionales en regímenes de bajos recursos a través de regularización implícita. Es barato (\(\approx 30\) min, sin entrenamiento adicional), ortogonal a la arquitectura del adaptador, y aplicable tanto a \(v_{2.1b}\) como al ganador del experimento Two-Adapter.
  ...
  \item[Ejecución de la validación participativa.] El protocolo y la planilla anonimizados ya están preparados, pero falta aplicarlos con hablantes nativos o revisores competentes. Esta etapa debe recoger puntajes de adecuación, fluidez y pertinencia cultural, analizar desacuerdos y contrastar los juicios humanos con chrF++ y las demás métricas automáticas.
...
El protocolo, implementado por \filepath{scripts/nmt/71\_human\_eval\_template.py}, se reproduce con el comando:

\begin{quote}
\texttt{.venv-nmt/Scripts/python -m scripts.nmt.71\_human\_eval\_template --variant xl --per-direction 100 --seed 2026 --split test}
\end{quote}

La ejecución prepara una muestra de 100 ítems por dirección (200 en total), comparando cuatro sistemas (\(v_0\), \(v_0\) reranked, \(v_{1bt}\) y \(v_{1bt}\) reranked) sobre el conjunto de \emph{test}. El muestreo es estratificado por origen (\emph{flashcards} vs \emph{pdf\_textos} vs otras fuentes) y por \emph{bucket} de longitud (corto \(\leq 5\), medio \(\leq 12\), largo \(>12\) palabras).

Las hipótesis se anonimizan con un mapeo aleatorio letra \(\to\) sistema (A, B, C, D), persistido en \filepath{reports/05\_nmt/evaluation\_xl/human\_eval\_anon\_key.json}. La planilla resultante \filepath{reports/05\_nmt/evaluation\_xl/human\_eval\_template.csv} contiene, por fila, la fuente, la referencia, las cuatro hipótesis anonimizadas y celdas vacías para que el revisor consigne puntajes 1--5 en las dimensiones de adecuación, fluidez y pertinencia cultural. El mismo script exporta además \filepath{reports/05\_nmt/evaluation\_xl/human\_eval\_protocol.md}, un resumen reproducible de alcance, muestra, rúbrica e instrucciones para revisores. Este anexo documenta el instrumento preparado; no incluye promedios humanos, acuerdos interevaluador ni comentarios consolidados porque la aplicación con hablantes no se ejecutó en esta etapa.
...
\begin{itemize}
  \item \textbf{LoRA+ aislado es el ganador.} \(v_{2.1b}\) alcanza avg chrF++ \(=44{,}99\) con IC 95\,\% \([43{,}17, 46{,}96]\), sin solapamiento con el IC de \(v_{1bt}\) (\([30{,}63, 33{,}80]\)). El salto de \(\approx 12{,}8\) puntos es la mejora más grande del ciclo y es estadísticamente clara.
  ...
  \item \textbf{DoRA+LoRA+ no mejora sobre LoRA+ aislado.} \(v_{2.1}\) reporta \(44{,}30\) con IC \([42{,}49, 46{,}12]\), cuyo IC se traslapa fuertemente con el de \(v_{2.1b}\). La señal dominante del ciclo proviene del optimizador, no de la descomposición del adaptador.
...
74 & \filepath{scripts/nmt/74\_thesis\_tables\_phase6.py} & \filepath{thesis/latex/figuras/generated/nmt\_\{leaderboard,bootstrap\_ci,rare\_token\_full\}[\_xl].tex} & — \\
