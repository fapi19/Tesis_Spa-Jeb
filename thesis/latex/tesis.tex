...
Las causas y consecuencias siguientes sintetizan el diagnóstico sociolingüístico y técnico presentado en la problemática: la condición crítica del shiwilu y su baja presencia digital \parencite{valenzuela2018,unesco2023}, así como la escasez de recursos paralelos y la necesidad de evaluación reproducible en traducción automática de bajo recurso \parencite{mager2018,koehn2017}.
...
El modelado NMT bidireccional (R3--R4) se realiza con marcos de aprendizaje profundo de la familia Transformers y segmentación en subpalabras mediante SentencePiece, a fin de reducir OOV y estabilizar el vocabulario en ambas direcciones \parencite{kudo2018sentencepiece}. El entrenamiento se conduce bajo control experimental (semillas, \emph{checkpoints} y configuraciones registradas) y se evalúa con métricas reproducibles (BLEU y chrF++), garantizando comparabilidad entre modelos y corridas \parencite{papineni2002bleu,popovic2015chrf}.
...
En conjunto, estas herramientas sostienen cuatro principios operativos: calidad de datos (regex, tokenizador, embeddings e índice), robustez del modelo (SentencePiece y marcos de Deep learning), evaluación reproducible (BLEU/chrF++) y transferencia futura a la práctica (prototipo local y validación experta).
...
En conjunto, la evidencia señala que no existe una única solución, sino un conjunto de técnicas complementarias. La retrotraducción explota datos monolingües, el parafraseo introduce variabilidad semántica, los idiomas pivote y la transferencia multilingüe aportan conocimiento desde pares más ricos, los \emph{embeddings} permiten extraer y filtrar corpus adicionales, y la recopilación activa añade datos verificados por hablantes. La integración de estas estrategias ha demostrado mejorar sustancialmente la calidad de la traducción, incluso en contextos con recursos extremadamente limitados. Esta síntesis se desprende de los trabajos revisados en los párrafos anteriores.
...
\begin{description}[leftmargin=*]
  ...
  \item[Ejecución de la validación participativa.] El protocolo y la planilla para hablantes ya están preparados. Su aplicación con hablantes nativos o revisores competentes queda \textbf{todavía en desarrollo de acuerdo al cronograma}. Esta etapa recogerá puntajes de sentido y naturalidad, decisiones de aceptar/corregir/rechazar, comentarios cualitativos, análisis de desacuerdos y contraste entre juicios humanos y métricas automáticas.
...
La actualización se realiza in-place sobre el modelo redimensionado (\texttt{model.resize\_token\_embeddings}), respetando el atado de pesos entre \emph{input embeddings} y \emph{lm head} cuando aplica. La rutina se encuentra en \filepath{src/nmt/training/model\_setup.py}.

\subsection*{C.3 Filtrado semántico: configuración y umbrales}\label{anexo-nmt-filtro}

El filtro semántico utiliza el modelo congelado \texttt{v3\_iterative\_hn\_e5\_base\_bidirectional\_xl} para puntuar la similitud coseno entre fuente y destino de cada par paralelo. Por convención de E5, los textos se prefijan con \texttt{query:} y \texttt{passage:} respectivamente antes de la codificación. La normalización L2 se aplica antes de calcular el coseno, equivalente a producto interno con vectores unitarios.
...
Cada predicción persiste tres campos adicionales en \texttt{*\_predictions.jsonl}: \texttt{confidence\_score}, \texttt{confidence} \(\in\{\text{low}, \text{medium}, \text{high}\}\), \texttt{confidence\_components}.
...
Esta sección documenta la reproducción técnica del protocolo descrito en el Capítulo~\ref{protocolo-validacion-automatica-participativa}. El instrumento prepara la muestra, la planilla de captura y las instrucciones operativas; la recolección de respuestas humanas se encuentra \textbf{todavía en desarrollo de acuerdo al cronograma}.

El protocolo se implementa en \filepath{scripts/nmt/75\_human\_validation\_workbook.py} y se reproduce con el comando:

\begin{quote}
\texttt{.venv-nmt/Scripts/python -m scripts.nmt.75\_human\_validation\_workbook}
\end{quote}
...
\begin{longtable}[]{@{}
  >{\centering\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.0700}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.3300}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.3000}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.3000}}@{}}
